...
public sealed class CanonicalRepository
{
    ...
    public CanonicalRepository(HarnessDatabase database) => _database = database ?? throw new ArgumentNullException(nameof(database));
    ...
    public async Task<ResultPage<StoredItem>> GetHistoryAsync(long hostSessionId, PageRequest page, CancellationToken cancellationToken = default)
    {
        ...
        {
            var content = JsonSerializer.Deserialize<IReadOnlyList<ContentBlock>>(reader.GetString(7), JsonOptions)
        ...
        string? adapterId, string? profileId, string? nativeSessionId, PageRequest page, CancellationToken cancellationToken = default)
    {
        ...
        while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
            var item = JsonSerializer.Deserialize<KernelEvent>(reader.GetString(2), JsonOptions)
    ...
    public async Task UpsertApprovalAsync(StoredApproval approval, CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText = """
            ...
            ON CONFLICT(approval_id) DO UPDATE SET status=excluded.status,response_json=excluded.response_json,resolved_at=excluded.resolved_at;
            """;
    ...
    private static int DecodeCursor(string? cursor)
    {
        ...
        try
        {
            ...
            if (!int.TryParse(text, NumberStyles.None, CultureInfo.InvariantCulture, out var offset) || offset < 0) throw new FormatException();
            return offset;
        }
